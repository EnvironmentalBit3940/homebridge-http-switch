...
function HTTP_SWITCH(log, config) {
  ...
  this.switchType = utils.enumValueOf(
    SwitchType,
    config.switchType,
    SwitchType.STATEFUL,
  );
  ...
  if (
    ...
    typeof config.serialNumber === "string"
  ) {
  ...
  if (this.switchType === SwitchType.STATEFUL) {
    ...
    if (config.statusPattern) {
      if (typeof config.statusPattern === "string")
        ...
        this.log.warn(
          "Property 'statusPattern' was given in an unsupported type. Using default one!",
        );
    }
    ...
    if (config.statusCache && typeof config.statusCache !== "number")
      this.log.warn(
        "Property 'statusCache' was given in an unsupported type. Using default one!",
      );
  }

  /** @namespace config.multipleUrlExecutionStrategy */
  if (config.multipleUrlExecutionStrategy) {
    const result = http.setMultipleUrlExecutionStrategy(
      config.multipleUrlExecutionStrategy,
    );

    if (!result)
      this.log.warn(
        "'multipleUrlExecutionStrategy' has an invalid value (" +
          config.multipleUrlExecutionStrategy +
  ...
  if (config.httpMethod) {
    // if we have it defined globally override the existing one of ON and OFF config object
    this.log(
      "Global 'httpMethod' is specified. Overriding method of on and off!",
    );
    if (this.on)
  ...
  const onCharacteristic = this.homebridgeService
  ...
  if (config.pullInterval) {
    if (this.switchType === SwitchType.STATEFUL) {
      ...
      this.log(
        "'pullInterval' was specified, however switch is stateless. Ignoring property and not enabling pull updates!",
      );
  }

  if (config.notificationID) {
    if (
      this.switchType === SwitchType.STATEFUL ||
      this.switchType === SwitchType.TOGGLE ||
      this.switchType === SwitchType.TOGGLE_REVERSE
    ) {
      ...
      this.log(
        "'notificationID' was specified, however switch is stateless. Ignoring property and not enabling notifications!",
      );
  }
    ...
    if (
      ...
      if (options) {
        try {
          ...
          this.log.error(
            "Error occurred creating mqtt client: " + error.message,
          );
        }
      }
    } else
      this.log(
        "'mqtt' options were specified, however switch is stateless. Ignoring it!",
      );
  }
  ...
  if (this.debug) {
    ...
    if (this.auth) this.log("  - auth options: " + JSON.stringify(this.auth));

    if (this.on) this.log("  - onUrls: " + JSON.stringify(this.on));
    if (this.off) this.log("  - offUrls: " + JSON.stringify(this.off));
    if (this.status) this.log("  - statusUrl: " + JSON.stringify(this.status));

    if (
      this.switchType === SwitchType.STATELESS ||
      this.switchType === SwitchType.STATELESS_REVERSE
    )
    ...
    if (this.mqttClient) {
      ...
      this.log(
        `  - mqtt client instantiated: ${options.protocol}://${options.host}:${options.port}`,
      );
      this.log("     -> subscribing to topics:");

      for (const topic in this.mqttClient.subscriptions) {
        if (!this.mqttClient.subscriptions.hasOwnProperty(topic)) continue;
...
HTTP_SWITCH.prototype = {
    ...
    if (this.switchType !== SwitchType.STATELESS_REVERSE) {
      if (config.onUrl) {
        try {
          ...
          this.log.warn(
            "Error occurred while parsing 'onUrl': " + error.message,
          );
          return false;
        }
      } else {
        this.log.warn(
          `Property 'onUrl' is required when using switchType '${this.switchType}'`,
        );
        return false;
      }
    } else if (config.onUrl)
      this.log.warn(
        `Property 'onUrl' is defined though it is not used with switchType ${this.switchType}. Ignoring it!`,
      );
    ...
    if (this.switchType !== SwitchType.STATELESS) {
      if (config.offUrl) {
        try {
          ...
          this.log.warn(
            "Error occurred while parsing 'offUrl': " + error.message,
          );
          return false;
        }
      } else {
        this.log.warn(
          `Property 'offUrl' is required when using switchType '${this.switchType}'`,
        );
        return false;
      }
    } else if (config.offUrl)
      this.log.warn(
        `Property 'offUrl' is defined though it is not used with switchType ${this.switchType}. Ignoring it!`,
      );
    ...
    if (this.switchType === SwitchType.STATEFUL) {
      ...
      if (config.statusUrl) {
        try {
          ...
          this.log.warn(
            "Error occurred while parsing 'statusUrl': " + error.message,
          );
          return false;
        }
      } else {
        this.log.warn(
          `Property 'statusUrl' is required when using switchType '${this.switchType}'`,
        );
        return false;
      }
    } else if (config.statusUrl)
      this.log.warn(
        `Property 'statusUrl' is defined though it is not used with switchType ${this.switchType}. Ignoring it!`,
      );
    ...
    switch (this.switchType) {
      case SwitchType.STATEFUL:
        if (!this.statusCache.shouldQuery()) {
          const value = this.homebridgeService.getCharacteristic(
            Characteristic.On,
          ).value;
          if (this.debug)
            this.log(
              `getStatus() returning cached value '${value ? "ON" : "OFF"}'${this.statusCache.isInfinite() ? " (infinite cache)" : ""}`,
            );
          callback(null, value);
          break;
        }

        if (this.debug) this.log("getStatus() doing http request...");
        ...
        http.httpRequest(this.status, (error, response, body) => {
          if (error) {
            ...
            !(
              http.isHttpSuccessCode(response.statusCode) ||
              http.isHttpRedirectCode(response.statusCode)
            )
          ) {
            this.log(
              "getStatus() http request returned http error code: %s",
              response.statusCode,
            );
            ...
            if (this.debug)
              this.log(
                `getStatus() request returned successfully (${response.statusCode}). Body: '${body}'`,
              );

            if (http.isHttpRedirectCode(response.statusCode)) {
              this.log(
                "getStatus() http request return with redirect status code (3xx). Accepting it anyways",
              );
            }
      ...
      case SwitchType.TOGGLE_REVERSE:
        callback(
          null,
          this.homebridgeService.getCharacteristic(Characteristic.On).value,
        );
        break;
  ...
  _makeSetRequest: function (on, callback) {
    ...
    if (this.debug) this.log("setStatus() doing http request...");
    ...
    http.multipleHttpRequests(urlObjectArray, (results) => {
      ...
      results.forEach((result, i) => {
        if (result.error) {
          ...
          !(
            http.isHttpSuccessCode(result.response.statusCode) ||
            http.isHttpRedirectCode(result.response.statusCode)
          )
        ) {
          errors.push({
            index: i,
            error: new Error(
              `HTTP request returned with error code ${result.response.statusCode}`,
            ),
            value: result.body,
          });
        } else {
          if (http.isHttpRedirectCode(result.response.statusCode)) {
            this.log(
              "setStatus() http request return with redirect status code (3xx). Accepting it anyways",
            );
          }
      ...
      if (errors.length > 0) {
        if (successes.length === 0) {
          if (errors.length === 1) {
            ...
            if (
              errorMessage &&
              !errorMessage.startsWith("HTTP request returned with error code ")
            )
            ...
            this.log(
              `Error occurred setting state of switch with every request (${errors.length}):`,
            );
            this.log(errors);
          }
        } else {
          this.log(
            `${successes.length} requests successfully set switch to ${on ? "ON" : "OFF"}; ${errors.length} encountered and error:`,
          );
          this.log(errors);
        }

        callback(
          new Error("Some or every request returned with an error. See above!"),
        );
      } else {
        if (this.debug)
          this.log(
            `Successfully set switch to ${on ? "ON" : "OFF"}${successes.length > 1 ? ` with every request (${successes.length})` : ""}`,
          );
        callback();
      }
